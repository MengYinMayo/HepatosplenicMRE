...
function I = getMREMagnitudeForROI(app, sl)
        ...
        % Interactive confidence threshold updates scatter and mean profile live.
            ...
            while true
                ...
                % Step 3: 0.5-mm bins, ignore first 10 mm
                validIdx = relDist > 10;
                rd2 = relDist(validIdx); rv2 = relVal(validIdx);
                ...
                while redoRange
                    ...
                    if strcmp(action,'accept') && isfinite(slope)
                        needContour = false;
                    elseif strcmp(action,'redo_range')
                        redoRange = true;
...
% Map internal Role string to friendly display name for study browser.
    ...
    map = struct( ...
        ...
        'SIEMENS_MRE_ConfMap',   'ConfMap');
    if isfield(map, role)
        lbl = map.(role);
    else
        lbl = strrep(strrep(strrep(role, 'GRE_', ''), 'EPI_', ''), 'SIEMENS_MRE_', '');
    end
end
