...
% =========================================================================
    ...
    properties (Access = public)
        ...
        BtnStiffGrad        matlab.ui.control.Button
    ...
    properties (Access = public)
        AppData struct = struct( ...
            ...
            'LossMax',      2, ...    % default loss modulus display half-range (W/L)
    ...
    methods (Access = private)
        ...
        function createMRETab(app)
            ...
            app.AxMREStorage.ButtonDownFcn = @(~,~)app.onMREPanelClick('storage');
            ...
            app.AxMRELoss.ButtonDownFcn = @(~,~)app.onMREPanelClick('loss');
            ...
            app.AxMRERawWave.Layout.Row = 2; app.AxMRERawWave.Layout.Column = 1;
            ...
            % Nav row at row 3 (44 px) — same height as colorbars in left/mid columns
            navG = uigridlayout(rightColG,[1 3]);
            navG.Layout.Row = 3; navG.Layout.Column = 1;
            navG.ColumnWidth = {64,'1x',64};
            navG.Padding = [4 6 4 6];
            navG.ColumnSpacing = 8;
            ...
            app.BtnStiff20.Layout.Column=2;
            ...
            % Middle column controls: ▶ Play wave (left, spanning rows) | Waves W/L [val] / Loss W/L [val]
            ctrlM = uigridlayout(imgG,[2 3]);
            ctrlM.Layout.Row=4; ctrlM.Layout.Column=2;
            ctrlM.ColumnWidth={86,'1x',72};
            ...
            app.BtnMREPlay.Layout.Row=[1 2]; app.BtnMREPlay.Layout.Column=1;
            app.BtnMREPlay.Text='▶ Play wave';
            app.BtnMREPlay.FontSize=11; app.BtnMREPlay.FontWeight='bold';
            ...
            wlbl = uilabel(ctrlM); wlbl.Layout.Row=1; wlbl.Layout.Column=2;
            wlbl.Text='Waves W/L'; wlbl.FontSize=11; wlbl.HorizontalAlignment='right';

            app.EdtWaveMax = uieditfield(ctrlM,'numeric');
            app.EdtWaveMax.Layout.Row=1; app.EdtWaveMax.Layout.Column=3;
            ...
            llbl = uilabel(ctrlM); llbl.Layout.Row=2; llbl.Layout.Column=2;
            llbl.Text='Loss W/L'; llbl.FontSize=11; llbl.HorizontalAlignment='right';

            app.EdtLossMax = uieditfield(ctrlM,'numeric');
            app.EdtLossMax.Layout.Row=2; app.EdtLossMax.Layout.Column=3;
            ...
            roiPnl = uipanel(app.MREGrid,'Title','MRE ROI Tools', ...
                'FontSize',12,'FontWeight','bold');
            roiPnl.Layout.Column = 2;
            rg = uigridlayout(roiPnl,[15 1]);
            rg.RowHeight   = {24,36,24,36,24,36,24,36,24,22,24,36,36,'1x',36};
            rg.Padding=[4 4 4 4]; rg.RowSpacing=4;
            ...
            app.BtnStiffGrad = uibutton(rg,'push');
            app.BtnStiffGrad.Layout.Row = 13;
            app.BtnStiffGrad.Text = 'Stiffness Gradient (kPa/mm)';
            app.BtnStiffGrad.FontSize = 11; app.BtnStiffGrad.FontWeight = 'bold';
            app.BtnStiffGrad.BackgroundColor = [0.10 0.45 0.35];
            app.BtnStiffGrad.FontColor = [1 1 1];
            app.BtnStiffGrad.Tooltip = 'Measure liver stiffness gradient: draw capsule contour and mark IVC';
            app.BtnStiffGrad.ButtonPushedFcn = @(~,~)app.onStiffGradBtn();

            app.LblMREInfo = uilabel(rg);
            app.LblMREInfo.Layout.Row=14;
            ...
            app.BtnClearMREROIs = roiBtn(rg,15,'Clear this slice',...
                [0.72 0.72 0.72],[0.2 0.2 0.2]);
...
function setCurrentMRETargetAxis(app, axisKey)
    ...
    validKeys = {'mag','raw','proc','stiff','storage','loss'};
    ...
    try
        ...
        while ~isempty(obj)
            ...
            elseif isequal(obj, app.AxMREStiff)
                axisKey = 'stiff'; break
            elseif isequal(obj, app.AxMREStorage)
                axisKey = 'storage'; break
            elseif isequal(obj, app.AxMRELoss)
                axisKey = 'loss'; break
            end
            try
                obj = obj.Parent;
...
function I = getMREMagnitudeForROI(app, sl)
        ...
        % Supports multi-slice: per-slice results stored in AppData.StiffGradData.
            ...
            Mag = [];
            try
                if isfield(mre,'M_raw') && ~isempty(mre.M_raw)
                    Mag = double(squeeze(mre.M_raw(:,:,min(sl,size(mre.M_raw,3)),1)));
                elseif isfield(mre,'M') && ~isempty(mre.M)
                    if ndims(mre.M)==4
                        Mag = double(mre.M(:,:,min(sl,size(mre.M,3)),1));
                    else
                        Mag = double(mre.M(:,:,min(sl,size(mre.M,3))));
                    end
                end
            catch; end
            ...
            pixSp = 1;
            try
                if isfield(mre,'H') && isfield(mre.H,'PixelSpacing')
                    pixSp = double(mre.H.PixelSpacing(1));
                end
            catch; end
            ...
            % ── Step 3: bin into 0.5-mm distance bins (ignore first 10 mm) ──
            validIdx  = relDist > 10;
            rd2 = relDist(validIdx);
            rv2 = relVal(validIdx);
            ...
            for di = 1:numel(uniqueD)
                ...
                idx = roundedD == d;
                if nnz(idx) >= 10
                    meanDist(end+1,1)  = d;              %#ok<AGROW>
                    meanStiff(end+1,1) = mean(rv2(idx)); %#ok<AGROW>
                    stdStiff(end+1,1)  = std(rv2(idx));  %#ok<AGROW>
                end
            end

            if isempty(meanDist)
                uialert(app.UIFigure, ...
                    'No distance bins had ≥10 reliable samples. Try a larger capsule region or lower confidence threshold.', ...
                    'Stiffness Gradient');
                return;
            end

            % ── Step 4: result figure + slope selection ─────────────────────
            fig2 = figure('Name', sprintf('Stiffness Gradient  Slice %d — select slope region', sl), ...
                'NumberTitle','off','Units','normalized','Position',[0.04 0.06 0.90 0.85]);
            ...
            hold(ax2,'off');
            title(ax2,'Stiffness map + profiles');
            ...
            grid on;
            xlabel('Distance from capsule to IVC (mm)'); ylabel('Stiffness (kPa)');
            title('Mean stiffness profile — click two points to define slope region');

            % User picks slope region
            figure(fig2); axes(hMean);
            [xSel, ~] = ginput(2);

            if numel(xSel) < 2 || ~isvalid(fig2)
                setStatus(app,'Stiffness gradient: slope selection cancelled.');
                return;
            end
            ...
            if nnz(segIdx) < 2
                uialert(app.UIFigure,'Too few points in the selected range.','Stiffness Gradient');
                hold(hMean,'off'); return;
            end

            xSeg = meanDist(segIdx);
            ySeg = meanStiff(segIdx);
            coeff = polyfit(xSeg, ySeg, 1);
            slope = coeff(1);
            ...
            hold(hMean,'off');
            title(hMean, sprintf('Mean stiffness profile  (slope = %.4f kPa/mm)', slope));
        ...
        % Notes column flags slices within 15 mm of a confirmed disc marker.
            try
                ...
                dixOrgans = { ...
                    ...
                    'PsoasDixon',  'Fat fraction (psoas muscle)'; ...
                    'TrunkDixon',  'Fat fraction (other skeletal muscles)'; ...
