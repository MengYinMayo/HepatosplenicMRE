...
function entry = classifySeries(entry)
    ...
    % Shield: GE EPI-MRE series often contain water/fat suppression tokens
    % ("FB Fat", "CSAT=WATER") that match the IDEAL-IQ keywords below.
    % Exclude any series whose description or sequence name signals MRE.
    ...
    if ~isMRELike && ...
       (hit(seq,  {'ideal3darc','ideal3d','idealarc','ideal'}) || ...
        ...
        elseif contains(desc,'mdixon') && ~contains(desc,'multi')
